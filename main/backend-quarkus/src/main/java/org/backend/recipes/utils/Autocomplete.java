...
public class Autocomplete {
    ...
    public Autocomplete(List<Term> termsInput) {
        if (termsInput == null) {
            throw new IllegalArgumentException("terms array cannot be null");
        }

        this.termCount = termsInput.size();
        // 1. Store and Sort the original array for BinarySearchDeluxe
        this.terms = new Term[this.termCount];
        for(int i = 0; i < termCount; ++i){
            if(termsInput.get(i) == null) throw new IllegalArgumentException("Term at index " + i + " is null");
            this.terms[i] = termsInput.get(i); // Defensive copy
        }
        try {
        ...
        if (len > 0) { // Comparator needs length > 0
            ...
            if (start != -1) {
                ...
                for (int i = start; i <= end; i++) {
                    if(uniqueResults.contains(this.terms[i])) continue;
